...
void knnsearch_parallel(double* C, double* Q, int num_points, int dimensions, int k, int* indices, double* distances) {
    ...
    // Use OpenCilk tasks to perform the searches in parallel
    cilk_spawn knnsearchBLAS(C, Q, half_points, dimensions, k, indices_half1, distances_half1);
    cilk_spawn knnsearchBLAS(C + half_points * dimensions, Q, half_points, dimensions, k, indices_half2, distances_half2);

    // Synchronize to ensure both tasks complete before proceeding
    cilk_sync;
